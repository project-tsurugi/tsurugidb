...
mkdir -p build
cd build
JAVA_HOME="${TG_TSUBAKURO_NATIVE_JAVA_HOME}" cmake -G Ninja -DCMAKE_BUILD_TYPE=${TG_CMAKE_BUILD_TYPE} -DCMAKE_CXX_FLAGS_RELWITHDEBINFO="${TG_CMAKE_CXX_FLAGS_RELWITHDEBINFO}" -DCMAKE_PREFIX_PATH="${TG_INSTALL_DIR}" -DCMAKE_INSTALL_PREFIX="${TG_INSTALL_DIR}" -DTSUBAKURO_LIBRARY_VERSION=${_TSUBAKURO_LIBRARY_VERSION} ${TG_COMMON_CMAKE_BUILD_OPTIONS} ..
