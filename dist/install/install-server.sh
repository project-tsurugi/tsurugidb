...
mkdir -p build
cd build
cmake -G Ninja -DCMAKE_BUILD_TYPE=${TG_CMAKE_BUILD_TYPE} -DCMAKE_CXX_FLAGS_RELWITHDEBINFO="${TG_CMAKE_CXX_FLAGS_RELWITHDEBINFO}" -DBUILD_TESTS=OFF -DBUILD_EXAMPLES=OFF -DSHARKSFIN_IMPLEMENTATION=shirakami -DCMAKE_PREFIX_PATH="${TG_INSTALL_DIR}" -DCMAKE_INSTALL_PREFIX="${TG_INSTALL_DIR}" -DCMAKE_MODULE_PATH="${TG_INSTALL_DIR}" -DBUILD_BRIDGE_ONLY=ON ${TG_COMMON_CMAKE_BUILD_OPTIONS} ..
cmake --build . --target install --parallel ${TG_CMAKE_BUILD_PARALLEL} -- ${TG_COMMON_BUILD_TOOL_OPTIONS}
...
mkdir -p build
cd build
cmake -G Ninja -DCMAKE_BUILD_TYPE=${TG_CMAKE_BUILD_TYPE} -DCMAKE_CXX_FLAGS_RELWITHDEBINFO="${TG_CMAKE_CXX_FLAGS_RELWITHDEBINFO}" -DBUILD_TESTS=OFF -DSHARKSFIN_IMPLEMENTATION=shirakami -DOGAWAYAMA=${OPT_OGAWAYAMA} -DCMAKE_PREFIX_PATH="${TG_INSTALL_DIR}" -DCMAKE_INSTALL_PREFIX="${TG_INSTALL_DIR}" -DENABLE_JEMALLOC=${TG_ENABLE_JEMALLOC} ${TG_COMMON_CMAKE_BUILD_OPTIONS} ..
cmake --build . --target install --parallel ${TG_CMAKE_BUILD_PARALLEL} -- ${TG_COMMON_BUILD_TOOL_OPTIONS}
