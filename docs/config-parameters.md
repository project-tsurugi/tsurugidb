...
| `scan_default_parallel` | Integer | The maximum degree of parallelism for RTX scan tasks. The default value is 4. | The value must be equal to or less than `max_result_set_writers`. If the value is set to 0, parallel execution of RTX scan tasks will be disabled. |
...
|`secure`| Boolean (true/false) | Whether to use a secure communication channel for gRPC by default. The default value is `false`. | **As of version 0.1.0, `secure=true` is not supported. This parameter must remain `false`.** |
