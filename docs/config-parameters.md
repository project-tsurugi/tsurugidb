...
| `scan_default_parallel` | Integer | The maximum degree of parallelism for RTX scan tasks. The default value is 4. | This setting is experimental and may undergo changes in future releases. The value must be equal to or less than `max_result_set_writers`. |
